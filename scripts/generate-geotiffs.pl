...
if($failed) {
    ...
    print FAILEDJOBS 'gradsScript|ctlObs|obsVar|ctlClimo|climoVar|regrid|nFields|fields|dateOffset|period|archiveRoot|fileRoot'."\n";
}
...
JOB: foreach my $job (@jobs) {
    ...
    my($gradsScript,$ctlObs,$obsVar,$ctlClimo,$climoVar,$regrid,$nFields,$fields,$dateOffset,$period,$archiveRoot,$fileroot) = split(/\|/,$job);
    ...
    my $gradsErr = grads("run $gradsScript $ctlObs $obsVar $ctlClimo $climoVar $regrid $nFields $fields $start $end $geotiffRoot");
