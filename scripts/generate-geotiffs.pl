...
if($failed) {
    ...
    print FAILEDJOBS 'ctlObs|obsVar|ctlClimo|climoVar|regrid|nFields|fields|dateOffset|period|archiveRoot|fileRoot'."\n";
}
...
JOB: foreach my $job (@jobs) {
    ...
    my($ctlObs,$obsVar,$ctlClimo,$climoVar,$regrid,$nFields,$fields,$dateOffset,$period,$archiveRoot,$fileroot) = split(/\|/,$job);
    ...
    my $gradsErr = grads("run generate_geotiffs.gs $ctlObs $obsVar $ctlClimo $climoVar $regrid $nFields $fields $start $end $geotiffRoot");
